...
public class ShuduUI extends JFrame implements ActionListener{//创建数独主界面
	...
	public void actionPerformed(ActionEvent e){
		...
		if(e.getSource()==itemArray[0]){//初级
			JOptionPane.showMessageDialog(this, "等级模块正在开发中，敬请留意", "提示", JOptionPane.INFORMATION_MESSAGE);
		}
		...
		if(e.getSource()==jbArray[0]){//开始信的数独
			clock.start();
			/*该功能正在开发中*/
		}
		...
		if(e.getSource()==jbArray[3]){//重来,对原来的数独重新填写
			clock.start();
			jp.removeAll();
			jp.add(new Grid());
			jp.validate();
