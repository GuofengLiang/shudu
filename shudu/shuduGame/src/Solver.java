...
{	
	...
	public void initialStack(){//将每个空格里可以填的数据存放到栈中	
			int layer=0;												
			for(int i=0;i<9;i++)             //将结点与二维坐标建立联系－－－－即一维与二维的转化，以下我把每结点都看做解空间层结点
			{
				for(int j=0;j<9;j++)
				{
					if(data[i][j]==0)
					{
						node[layer][0]=i;//0和1用来区分横纵坐标
						node[layer][1]=j;
						layer++;
					}
				}
			...
			for(int num=1;num<=9;num++)  
			{
				for(int i=0;i<n;i++)	//对每一层的可能值进行判断
				{
					...
					{
						stack[i][num-1]=1;//表示第i层该数字能填，通过stack暂时存储所有可能值，num-1最大只能到8
						count[i]++;//这样容易记录每一层可能值的个数
					}
				}
			}
			int[][] temp=new int[n][];    //借助该二维数组存储每一层解空间具体可能的值
			for(int i=0;i<n;i++)          //将chance的1和0代表的可能值转化为具体数字
			{
				temp[i]=new int[count[i]];//count[i]这里表示可填数的个数----这里的层对应二维数组temp中的行
				int p=0;				//表示每一层都要从头开始遍历
				for(int j=0;j<9;j++)
				{
					if(stack[i][j]==1)//如果可以填j+1这个数字
					{
						temp[i][p]=j+1;//j+1表示的是存放的数字，这里表示按层遍历赋值，对每层每个数进行具体的赋值
						p++;//p表示每层能填的数字的个数,这个值不是固定的，跟着循环
					}
				}
			}
			stack=temp; //可能值的具体表示赋给stack
	}
